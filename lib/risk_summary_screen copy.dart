...
class _RiskTrendsScreenState extends State<RiskTrendsScreen> {
  ...
  _ChartData _chartData() {
    ...
    if (_selectedFilter == 'รายสัปดาห์') {
      ...
      for (var i = 11; i >= 0; i--) {
        ...
        labels.add(
          '${start.month}/${(start.year % 100).toString().padLeft(2, '0')}',
        );
      }
    }
  ...
  Map<String, int> _breakdown(_ChartData data) {
    ...
    for (final alert in _alerts) {
      final time = _alertTime(alert);
      if (time == null || time.isBefore(data.start) || !time.isBefore(data.end))
        continue;
          ...
          child: Column(
            ...
            children: [
              ...
              if (_isLoading)
                ...
                _buildRiskBreakdownGrid(_breakdown(chart)),
  ...
  Widget _buildRiskBreakdownHeader() => Text(
  ...
  Widget _buildRiskBreakdownGrid(Map<String, int> types) {
    if (types.isEmpty)
      return Center(
        child: Padding(
          padding: const EdgeInsets.all(24),
          child: Text(
            'ไม่พบ Alert ในช่วงเวลานี้',
            style: GoogleFonts.prompt(color: Colors.grey.shade600),
          ),
        ),
      );
    return GridView.count(
      ...
      children: types.entries
          .map((entry) => _buildBreakdownCard(entry.key, entry.value))
          .toList(),
    );
  }
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: (meta.color as Color).withOpacity(.12),
              borderRadius: BorderRadius.circular(10),
            ),
            child: Icon(meta.icon as IconData, color: meta.color as Color),
          ),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
  ...
  _AlertMeta _alertMeta(String type) {
    switch (type) {
      case 'ง่วงนอน':
        return const _AlertMeta(Icons.bedtime_rounded, Colors.red);
      case 'เสียสมาธิ':
      case 'ไม่มองถนน':
        return const _AlertMeta(Icons.visibility_off_rounded, Colors.orange);
      case 'ใช้โทรศัพท์':
        return const _AlertMeta(Icons.phone_android_rounded, Colors.deepPurple);
      case 'ขับรถเร็ว':
        return const _AlertMeta(Icons.speed_rounded, Colors.blue);
      case 'เบรกกะทันหัน':
        return const _AlertMeta(Icons.warning_rounded, Colors.amber);
      default:
    ...
    final points = <Offset>[
      for (var i = 0; i < values.length; i++)
        Offset(
          ...
          size.height - (values[i] / maxY) * (size.height - 16) - 8,
        ),
    ];
